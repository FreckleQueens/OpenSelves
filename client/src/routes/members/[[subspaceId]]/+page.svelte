<script lang="ts">
	...
	import { subscribeToModel } from "$lib/idb/entry-subscription.svelte.js";
	...
	const SHOW_ARCHIVED_MEMBERS_SETTINGS_KEY = "showArchivedMembers";

	let { params }: PageProps = $props();

	// svelte-ignore state_referenced_locally
	const subspaceId: SubspaceId = params.subspaceId
	...
	let fronts = $derived.by(subscribeToModel(Front, subspaceId));
	...
	onMount(async () => {
		showArchivedMembers = !!(await Settings.get(SHOW_ARCHIVED_MEMBERS_SETTINGS_KEY));
	});

	async function addMemberButtonOnClick() {
		await goto(resolve(`/members/${subspaceId.toHex()}/edit/`));
	}

	async function toggleShowArchivedMembers() {
		showArchivedMembers = !showArchivedMembers;
		await Settings.set(SHOW_ARCHIVED_MEMBERS_SETTINGS_KEY, showArchivedMembers ? "on" : "");
	}
...
>
		...
		{#each currentFronts as front (front.id)}
			<MemberCard
				...
				onClick={() => {
					if (front.member)
						return goto(
							resolve(`/members/${subspaceId.toHex()}/edit/${front.member.id}`),
						);
				}}
				actions={[
					{
				...
				secondaryActions={[
					...
					},
					{
						...
						icon: EditIcon,
						onClick: () =>
							goto(resolve(`/members/${subspaceId.toHex()}/front/edit/${front.id}`)),
					},
				]}
			>
				{#snippet chips()}
	...
	<Block id="not-fronting-members">
		{#each shownMembers as member (member.id)}
			<MemberCard
				{member}
				onClick={() => goto(resolve(`/members//${subspaceId.toHex()}/edit/${member.id}`))}
				actions={[
					{
						id: "start-front",
						icon: JoinFrontIcon,
	...
	{#snippet bottomNav()}
		<FabMenu
			...
			{pageContent}
		/>
		<MembersTabbar {subspaceId} activeTab={MembersTab.CURRENT} />
	{/snippet}
</AppPage>
